...
The ``description`` output can be seen by executing the following console
command::
...
To illustrate the ``epilog()`` method in action lets add a call to
the ``getOptionParser()`` method used above::
